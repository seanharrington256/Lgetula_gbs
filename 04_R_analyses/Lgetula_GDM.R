### Script to run generalised dissimilarity modelling (GDM) on L. getula GBS data

### CHANGE PATHS IF YOU WANT TO USE THIS SCRIPT!!!!! These paths exist on my computer, but almost definitely not on yours
...
# read in environmental data as extracted in the Lgetula_IBD.R script
alt_data <- "all_coords_altitude.csv"
...
### Set this up so that major code blocks only need to be written once
######################################################################################################################
######################################################################################################################
...
for(species in assemblies_gdm){   ### loop over all assemblies
  ...
  # match environmental data to the order of the individuals in genetic data (also prunes these data to just those for the assembly of interest)
  ...
  # instead of the commented out automatic way above, I have manually done this in the Lgetula_IBD.R script
